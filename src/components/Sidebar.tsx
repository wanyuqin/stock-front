...
const NAV_ITEMS = [
  {
    ...
    items: [
      ...
      { to: '/market',    icon: TrendingUp,      label: '行情中心', badge: 'BETA' },
    ],
  },
  {
    group: '账户',
    items: [
      ...
      { to: '/analysis',  icon: Activity,    label: 'AI 分析',  badge: 'NEW' },
    ],
  },
]
    ...
    <div className="px-4 py-5 border-b border-terminal-border">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <p className="text-ink-primary font-semibold text-sm leading-tight tracking-wide">
            A股分析
          </p>
        ...
        <div className="flex items-center gap-1.5">
          <span
            className={`w-1.5 h-1.5 rounded-full ${
              isOpen ? 'bg-accent-green animate-pulse' : 'bg-ink-muted'
            }`}
          />
          <span
            className={`text-xs font-mono ${
              isOpen ? 'text-accent-green' : 'text-ink-muted'
            }`}
          >
            {isOpen ? '交易中' : '已收盘'}
          </span>
      ...
      >
        ...
        <Icon
          ...
          className={`flex-shrink-0 transition-colors ${
            isActive ? 'text-accent-green' : 'text-ink-muted group-hover:text-ink-secondary'
          }`}
        ...
        {badge && (
          <span className={`
            text-[10px] font-mono px-1.5 py-0.5 rounded font-medium tracking-wide
            ${badge === 'NEW'
  ...
  return (
    <div className="border-t border-terminal-border p-3">
      ...
      <div className="px-2 py-2 mb-2">
        <p className="text-ink-muted text-[11px] font-mono mb-1.5 uppercase tracking-widest">
          快捷键
        </p>
      ...
      <button className="
        ...
        text-ink-muted hover:text-ink-secondary hover:bg-terminal-muted
...
export default function Sidebar() {
  return (
    <aside className="
      w-52 flex-shrink-0 h-screen flex flex-col
      bg-terminal-panel border-r border-terminal-border
      animate-slide-in
    ">
      <SidebarLogo />
      <MarketStatus />
