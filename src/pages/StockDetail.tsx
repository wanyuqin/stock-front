...
import {
  ...
  BrainCircuit, Banknote, PieChart,
} from 'lucide-react'
...
// fetchBigDeal 已在 BigDealPanel 内部按需调用
import {
...
function KLineToolbar({ period, onChange, loading }: { period: Period; onChange: (p: Period) => void; loading: boolean }) {
  return (
    <div className="flex items-center gap-1">
      ...
      {([{ label: '3月', value: 60 }, { label: '6月', value: 120 }, { label: '1年', value: 250 }] as { label: string; value: Period }[]).map(o => (
        <button key={o.value} onClick={() => onChange(o.value)}
          className={`px-2 py-1 rounded text-xs font-mono transition-all ${period === o.value ? 'bg-terminal-muted border border-terminal-border text-ink-primary' : 'text-ink-muted hover:text-ink-secondary'}`}>
          {o.label}
        </button>
      ))}
    </div>
...
function MoneyFlowPanel({ code }: { code: string }) {
  ...
  const { data, loading, error } = useQuery(useCallback(() => fetchMoneyFlow(code, 10), [code, histKey]))
  ...
  const mainInflow  = liveData ? Number(liveData.main_net_inflow)    : latest?.main_net_inflow ?? null
  const superInflow = liveData ? Number(liveData.super_large_inflow) : latest?.super_large_inflow ?? null
  const largeInflow = liveData ? Number(liveData.large_inflow)       : latest?.large_inflow ?? null
  const pct         = liveData ? Number(liveData.main_inflow_pct)    : latest?.main_inflow_pct ?? null
  const updatedAt   = liveData ? '刚刚' : latest ? new Date(latest.created_at).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }) : null
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-terminal-border flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          {updatedAt && <span className="text-[10px] font-mono text-ink-muted">更新 {updatedAt}</span>}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {loading && logs.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-ink-muted gap-3">
            ...
            <p className="text-xs font-mono text-center">暂无资金流向数据</p>
            <button onClick={handleRefresh} disabled={refreshing} className="btn-ghost text-xs disabled:opacity-40">
              {refreshing ? <><RefreshCw size={11} className="animate-spin" />抓取中…</> : '立即抓取实时数据'}
            </button>
          </div>
        ) : (
          <>
              ...
              {[
                ...
                <div key={label} className={`rounded-lg p-3 border ${highlight ? 'border-accent-green/30 bg-accent-green/5' : 'border-terminal-border bg-terminal-muted/40'}`}>
                  ...
                  {value !== null && value !== undefined
                    ? <p className={`text-sm font-mono font-bold ${inflowColor(value)}`}>{formatYuan(value)}</p>
                    : <p className={`text-sm font-mono font-bold ${(p ?? 0) >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>{(p ?? 0) >= 0 ? '+' : ''}{(p ?? 0).toFixed(2)}%</p>}
                </div>
              ))}
            </div>
            {mainInflow !== null && (
                  ...
                  return (
                    <div key={label} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-2 bg-terminal-muted rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-500 ${value >= 0 ? 'bg-accent-green/60' : 'bg-accent-red/60'}`} style={{ width: `${p}%` }} />
                      </div>
                      <span className={`text-[10px] font-mono w-16 text-right flex-shrink-0 ${inflowColor(value)}`}>{formatYuan(value)}</span>
                    </div>
                  )
                })}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="rounded-lg border border-terminal-border overflow-hidden">
                  <table className="w-full text-xs">
                    <thead><tr className="border-b border-terminal-border bg-terminal-muted">
                    ...
                    <tbody>
                      {logs.map(log => (
                        <tr key={log.id} className="border-b border-terminal-border/50 last:border-0">
                          <td className="px-2 py-1.5 font-mono text-ink-muted text-[10px]">{new Date(log.created_at).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}</td>
                          <td className={`px-2 py-1.5 font-mono text-right text-[10px] font-semibold ${inflowColor(log.main_net_inflow)}`}>{formatYuan(log.main_net_inflow)}</td>
                          <td className={`px-2 py-1.5 font-mono text-right text-[10px] ${inflowColor(log.main_inflow_pct)}`}>{log.main_inflow_pct >= 0 ? '+' : ''}{log.main_inflow_pct.toFixed(2)}%</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {error && <div className="px-4 pb-3 flex-shrink-0"><ErrorBanner message={error} /></div>}
    </div>
  )
}

// ── 右侧面板 Tab ──────────────────────────────────────────────────
type RightTab = 'ai' | 'moneyflow' | 'bigdeal' | 'valuation'
        ...
        <div className="flex-1 min-h-0 flex overflow-hidden">
          ...
          <div className="w-80 xl:w-96 flex-shrink-0 flex flex-col overflow-hidden">
            ...
            <div className="flex-shrink-0 flex border-b border-terminal-border">
              {([
                ...
                <button key={key} onClick={() => setRightTab(key)}
                  className={`flex-1 flex items-center justify-center gap-1 py-2.5 text-[11px] font-mono transition-colors border-b-2 ${
                    rightTab === key ? 'border-accent-cyan text-accent-cyan' : 'border-transparent text-ink-muted hover:text-ink-secondary'
                  }`}>
                  <Icon size={11} />{label}
                </button>
              ))}
            ...
            <div className="flex-1 overflow-hidden overflow-y-auto">
              {rightTab === 'ai' ? (
                <AIReportPanel data={analysis} loading={analysisLoading} error={analysisError} onRefresh={refetchAnalysis} />
              ) : rightTab === 'moneyflow' ? (
                <MoneyFlowPanel code={code} />
              ) : rightTab === 'bigdeal' ? (
                <BigDealPanel code={code} changeRate={quote?.change_rate} />
              ) : (
